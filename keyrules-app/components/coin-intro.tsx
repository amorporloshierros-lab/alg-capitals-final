...
export default function CoinIntro({ onDone }: { onDone?: () => void }) {
  const skip = typeof window !== 'undefined' && sessionStorage.getItem('kr_intro_done') === '1'
  const [phase, setPhase] = useState<'pre' | 'drop' | 'settle' | 'fade' | 'gone'>(skip ? 'gone' : 'pre')
  const [hidden, setHidden] = useState(skip)

  useEffect(() => {
    if (skip) { setHidden(true); onDone?.(); return }
    const t0 = setTimeout(() => setPhase('drop'), 150)
    const t1 = setTimeout(() => setPhase('settle'), 2450)
    const t2 = setTimeout(() => setPhase('fade'), 3900)
    const t3 = setTimeout(() => {
      setHidden(true)
      sessionStorage.setItem('kr_intro_done', '1')
      onDone?.()
    }, 5000)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ font: '900 italic 10px/1 var(--font-sans)', letterSpacing: '.6em', color: '#737373', textTransform: 'uppercase' }}>
            Keyrules × Alg Capitals
          </div>
          <div style={{ marginTop: 8, font: '400 10px/1 var(--font-mono)', letterSpacing: '.35em', color: '#404040', textTransform: 'uppercase' }}>
      ...
      <style>{`
        ...
        .coin-intro-face {
          ...
          box-shadow:0 0 0 2px rgba(16,185,129,.18), 0 0 60px rgba(16,185,129,.45), inset 0 0 30px rgba(0,0,0,.55);
        }
        .coin-intro-face.back { transform:rotateY(180deg); filter:brightness(.78) contrast(1.12); }
        .coin-intro-rim {
        ...
        .coin-intro-sheen {
          ...
          background:conic-gradient(from 200deg,
            ...
            rgba(255,255,255,.55) 90deg, rgba(16,185,129,.35) 110deg,
...
function MatrixRain({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    let raf: number
    let drops: number[], fontSize: number, cols: number, w: number, h: number, dpr: number
    ...
    const draw = () => {
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (let k = 1; k < 6; k++) {
          ...
          ctx.fillText(Math.random() > 0.5 ? '1' : '0', x, y - k * fontSize)
        }
...
function CornerTicks() {
  ...
  const tickBR = (pos: React.CSSProperties) => (
    <div style={{ position: 'absolute', width: 28, height: 28, ...pos }}>
